...
- AI：轴心国会使用指令点优先整补据点守军、在据点周边部署增援，评估高价值目标使用火炮弹幕或突破突击，并用完整移动范围压向据点或进入攻击位；回到盟军回合后侧栏会显示敌方回合摘要和行动时间线，地图用 AI 复盘标记显示移动起终点、攻击/战术目标、部署/整补和占点坐标，汇总并列出移动、攻击、战术命令、后勤、占点、歼灭、伤害和指令点变化
