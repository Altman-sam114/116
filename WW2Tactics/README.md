...
- UI：顶部回合栏、剩余回合、战局进度、三星目标、双方战力、编队条、战术指令条、地图缩放、战区坐标、按当前焦点排序的全友军/据点/全敌军快速定位、地图命令预览、单位详情、地形详情、将领加成、图例、战报、重新开始；v1.56 起首屏外壳使用统一战区指挥台视觉基线，顶栏、状态芯片、地图容器、HUD 背板和侧栏容器共享战场主题；v1.57 起地图格和单位棋子升级材质与状态层级；v1.58 起攻击目标、战斗预测和普通攻击结果卡统一为战报式反馈牌；v1.59 起战线态势面板重构为分区清晰的指挥简报；v1.60 起 AI 回合摘要、复盘结论、关键事件、播放控制和行动时间线统一为战况回放面板，并独立显示当前回放事件，播放进入前五条之外仍能在侧栏看到当前反馈；v1.61 起地图格标记改为统一顶/底槽位与角落命令角标，同格多标记按优先级折叠并显示 +N，避免压住单位棋子与可执行 MOVE/ATK/POS；v1.62 起地图内联命令预览与侧栏命令预览共用 `MapCommandPreviewChrome` 文案/图标/风险摘要 helper，减少重复实现；v1.63 起战术命令/据点占领/部署/整补结果卡对齐战斗结果战报卡视觉；v1.64 起反制执行回放与敌方回合复核卡对齐同一战报卡基线；v1.65 起顶栏状态条与地图 HUD 信息密度收紧，快捷命令点击区加大；v1.66 起侧栏 Inspector 分区层级与常用触控目标加大；v1.67 起图例与编队条统一到战区指挥台视觉；v1.68 起窄屏/中宽断点与地图 HUD 紧凑布局适配；v1.69 起战役/地形/战报面板对齐指挥台视觉并改善动态字体；v1.70 起单位详情分区层级与战术条/增援坞视觉统一；v1.71 起补给/士气/经验/威胁状态面板对齐指挥台视觉；v1.72 起目标计划/安全接敌/战术命令/将领面板提升可玩性反馈；v1.73 起敌方意图与反制建议面板对齐指挥台视觉；v1.74 起反制下一步提示与关联 AI 行动行增强可玩性反馈；v1.75 起态势响应卡与 AI 回放当前事件反馈强化；v1.76 起据点压力行与复盘影响入口强化反馈
