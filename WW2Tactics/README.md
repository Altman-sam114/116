...
- AI：轴心国会使用指令点优先整补据点守军、在据点周边部署增援，评估高价值目标使用火炮弹幕或突破突击；没有可击毁目标时会优先直取本回合可达的中立或盟军据点，再用完整移动范围压向据点或进入攻击位，移动后若普通攻击不可用但进入弹幕射程，火炮会继续用火炮弹幕压制目标；回到盟军回合后侧栏会显示敌方回合摘要和行动时间线，地图用 AI 复盘标记显示移动起终点、攻击/战术目标、部署/整补和占点坐标，时间线条目可点选定位到对应复盘坐标并保留当前事件选中态，也可用上一条/下一条连续查看敌方行动，或用慢/中/快三档自动播放并可随时暂停；地图同步强调当前事件的 AI 标记，但不会执行命令，汇总并列出移动、攻击、战术命令、后勤、占点、歼灭、伤害和指令点变化
