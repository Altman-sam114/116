...
- UI：顶部回合栏、剩余回合、战局进度、三星目标、双方战力、编队条、战术指令条、地图缩放、战区坐标、按当前焦点排序的全友军/据点/全敌军快速定位、地图命令预览、单位详情、地形详情、将领加成、图例、战报、重新开始；v1.56-v1.76 完成指挥台主题、地图标记槽位、结果卡、侧栏分区、响应式布局、态势响应与 AI 回放反馈统一；v2.0 将主题、chrome/HUD、地图/input 和单位视觉从 `ContentView` 拆为独立模块，地图取消卡片式厚阴影，以确定性田野、林带、城市道路、山脊、雪线、河道和公路纹理形成连续战区；四类单位使用具象轮廓、盟军圆角底座/轴心切角虚线底座、稳定 HP 条与通用将领徽章。v2.2 把顶栏固定为单行、移除地图内重复战役卡、默认收起支援甲板，普通地格只在焦点时显示地形代码；默认六角边线更弱，单位主体和 HP 条更大，地图首屏面积明显增加。快捷命令保持至少 44pt 点击区；滚动聚焦和支援甲板展开尊重 Reduce Motion，地图格和单位保留 VoiceOver 摘要。
