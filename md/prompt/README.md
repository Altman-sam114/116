...
当前已进入 `v2（六角格战争界面）`。v2.0 负责表现层文件拆分、连续地貌、具象单位和首屏 chrome 收敛；v2.1 增加云端模拟器首屏截图、artifact 视觉证据和截图失败门禁；v2.2 根据首张真实 iPad 截图收紧顶栏与支援区、降低地格噪音并放大单位。下一轮应根据 v2.2 新截图只选择“地貌细化”或“战斗动效”之一。
