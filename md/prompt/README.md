...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 收敛空闲 HUD 并加入战斗准星；v2.8 让地图军械模型脱离大面积徽章底板；v2.9 根据官方截图强化七类地貌纵深；v2.10 为蓝色选中反馈、橙色 POS 准星和动作 HUD 建立专用云端截图场景。下一轮应根据 v2.10 artifact 判断是否进入攻击判定卡和短促战斗反馈动效。
