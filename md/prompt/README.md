...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 对照官方战斗截图收敛空闲 HUD，并加入选中光环与 ATK/POS 四角准星；v2.8 根据 v2.7 云端截图让地图军械模型脱离大面积徽章底板，以透明地面环直接落在战区。下一轮应根据 v2.8 截图继续处理地貌纵深或战斗预览动效。
