...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 收敛空闲 HUD 并加入战斗准星；v2.8 让地图军械模型脱离大面积徽章底板；v2.9 强化地貌纵深；v2.10 建立选中/POS 云端截图场景；v2.11 根据该截图收敛非焦点 MOVE、ATK、POS 和 THR 标记；v2.12 进入真实 ATK 火力解算与云端截图验收。v2.12 通过后，下一轮优先根据 artifact 选择“短促战斗结果反馈”或“目标聚光与地图遮挡继续收敛”。
