...
当前已进入 `v2（六角格战争界面）`。v2.0 负责表现层文件拆分、连续地貌、具象单位和首屏 chrome 收敛；v2.1 增加云端模拟器首屏截图、artifact 视觉证据和截图失败门禁；v2.2 根据首张真实 iPad 截图收紧顶栏与支援区、降低地格噪音并放大单位；v2.3 根据第二张真实截图把常驻 Inspector 改为地图边缘按需展开的情报抽屉；v2.4 收敛重复地貌纹理并用坐标稳定变体增强连续地图感；v2.5 增强阵营军械色板、四类模型内部结构与分段 HP。下一轮应以 v2.5 截图为证据决定是否进入战斗动效。
