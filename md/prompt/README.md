...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 收敛空闲 HUD 并加入战斗准星；v2.8 让地图军械模型脱离大面积徽章底板；v2.9 根据官方截图和 v2.8 云端证据强化七类地貌的明暗面、投影与连续高光。下一轮应根据 v2.9 截图进入选中战斗态的云端视觉证据或攻击预览动效。
