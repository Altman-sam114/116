...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 收敛空闲 HUD 并加入战斗准星；v2.8 让地图军械模型脱离大面积徽章底板；v2.9 强化地貌纵深；v2.10 建立选中/POS 云端截图场景；v2.11 收敛非焦点 MOVE、ATK、POS 和 THR 标记；v2.12 完成真实 ATK 火力解算；v2.13 根据 v2.12 artifact 聚焦交战轴线并降低攻击态覆盖层噪音。v2.13 通过后，下一轮优先进入短促战斗结果反馈。
