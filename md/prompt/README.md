...
当前已进入 `v2（六角格战争界面）`。v2.0-v2.6 完成表现层拆分、云端截图链、地图优先布局、地貌自然化、单位结构和满幅战区；v2.7 收敛空闲 HUD 并加入战斗准星；v2.8 让地图军械模型脱离大面积徽章底板；v2.9 强化地貌纵深；v2.10 建立选中/POS 云端截图场景；v2.11 收敛非焦点 MOVE、ATK、POS 和 THR 标记；v2.12 完成真实 ATK 火力解算；v2.13 根据 v2.12 artifact 聚焦交战轴线并降低攻击态覆盖层噪音；v2.14 完成真实开火后的地图战损、反击与结果反馈；v2.15 对齐攻击前火力解算与真实结算；v2.16 建立命中冲击、HIT/RET 和结论出现节奏；v2.17 让地图战果短暂停留后自动淡出；v2.18 已将行动资源放到单位棋子底部、修正 completed 降权与 sibling 遮挡；v2.19 将选中单位 HUD 收敛为两行地图边缘动作坞；v2.20 当前轮把底部定位区重构为同时可见、分别横滚的 AL/OBJ/AX 三段，并以固定宽度模型、HP、行动状态和据点归属提高扫描效率。v2.20 通过后，可根据 artifact 选择继续拆分 `ContentView` 战场结果区，或独立改善紧凑轨道的焦点自动显露。
