...
- v2.18 的代码审查必须确认地图 `UnitCounter` 使用固定宽度的“移动图标 + 五段 HP + 开火图标”轨道，图标只读 `hasMoved/hasAttacked`，消耗后在原位切换完成符号且隐藏于无障碍树；整枚地图棋子仅在 `hasMoved && hasAttacked` 时降权，单独移动或攻击不能伪装成完全 spent。`--ci-selected-combat-impact-steady` 的真实完成态 M10 应能辨认左右完成符号，同时 PNG 继续满足 v2.16 战果内容与 500 KB 门禁。
