...
- v2.11 的云端截图必须与 v2.10 选中态对比：非焦点 MOVE 不再显示宽 `M5` 胶囊而使用紧凑成本圆点，非焦点 POS/ATK 使用图标，THR 不再重复文字；当前焦点仍保留完整命令语义，HUD 的 MOVE/ATK/THR/SUP 数量保持一致。代码审查确认只传递既有 `isFocused` 并改变标记绘制，不修改 `GameState`、actionHint、命中或 accessibility 主摘要。
