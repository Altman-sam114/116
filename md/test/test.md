...
- v2.46 的云端 regular `--ci-selected-combat-impact-steady` PNG 必须至少 500 KB 且非黑屏，并继续以六角地图、道路/河流和军械棋子为主工作区；普通 city 与 `TerrainTile.isObjective` 据点应有可读层级差异，盟军、轴心、中立据点应分别读出 `AL`、`AX`、`NEU` 或等价文字/几何语义，地标主体、旗杆/owner code、短名称牌和低对比接地强调不得变成大卡片墙。源码审查必须确认据点表现只从 `tile.isObjective`、`objectiveName`、`owner`、terrain 和确定性局部坐标派生；城市据点增强建筑簇，非城市据点使用轻量节点，带单位据点的名称牌不遮挡军械模型、HP/行动轨道或 selected。地图边缘、相邻据点、长名称、焦点状态、marker、道路/河流、HUD、v2.45 战果、`AL / OBJ / AX` rail、`ObjectiveJumpDock`/`focus(coordinate:)`、`HexInputReader`、tile frame/contentShape/zIndex/scroll/zoom 和完整 tile VoiceOver 摘要均不回归；所有新装饰必须 `allowsHitTesting(false)` 与 `accessibilityHidden(true)`，不新增按钮、gesture、状态、缓存或规则数据。workflow 没有 compact/窄宽度/`xxxLarge` Dynamic Type/多焦点/VoiceOver/Reduce Motion/独立 4x 输出，Agent C 只能作源码审查，不能把 regular PNG 冒充这些证据。
