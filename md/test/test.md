...
- 覆盖战役配置、地图、单位、移动、攻击、AI、补给、士气、目标推进、目标推进计划摘要、目标计划候选预览、安全接敌候选点选预览、敌方威胁意图预判、敌方意图反制建议、反制建议收益解释、排序对比解释、执行前后预计对照、反制建议点选聚焦、地图标记、执行入口桥接预览、执行回放、敌方回合复核、普通行动不生成或清理旧回放、重开/切战役清理回放、部署/整补结果摘要、AI 直取据点优先、AI 回合行动摘要、行动时间线、地图复盘标记、THR 威胁覆盖等主规则链。
...
- `GameStateTests.swift` 应通过源码级 typecheck，覆盖 AI 战术命令、移动后攻击、直取据点优先、敌方威胁意图预判、敌方意图反制建议、反制建议收益解释、排序对比解释、执行前后预计对照、反制建议点选聚焦、地图标记、执行入口桥接预览、执行回放、敌方回合复核、部署、整补、占点、歼灭、AI 回合行动时间线和地图复盘标记，以及重开/切战役清理 AI 回合行动摘要、行动时间线、地图复盘标记、反制执行回放和复核。
