...
- `BattlefieldMap.swift` 保持原有 `position(for:)`、tile frame、`Hexagon` 命中路径和 `HexInputReader` 三种输入链；连续田野、森林、城市、山地、雪地、河流与公路纹理只从 `TerrainTile` 和 `HexCoordinate.neighbors` 只读派生；地图外接 frame 按 viewport 高度计算满幅缩放；选中光环与目标准星只叠加绘制，不参与命中。
