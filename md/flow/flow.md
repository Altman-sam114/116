...
- 维护 `BattlefieldTheme`、通用 tactical surface 表现样式、地图材质 helper、战斗反馈卡、战线态势指挥简报和 AI 战况回放样式、地图标记槽位折叠，以及命令预览共享 `MapCommandPreviewChrome` helper；战术/占领/部署/整补结果卡与战斗结果卡共享战报卡视觉基线；反制执行回放与敌方回合复核卡同样对齐该基线；顶栏状态条与地图 HUD 采用更紧凑信息密度布局；侧栏 Inspector 以分区标题组织主操作/态势/结果/敌情/战报，并放大常用触控目标；图例与编队条统一到 `BattlefieldTheme` 指挥台视觉；主界面按宽度断点切换并排/上下布局，窄地图上 HUD 改为紧凑叠放；战役目标/地形详情/战报等基础面板使用更可缩放文案与指挥台卡片样式；单位详情按状态/作战方案/指挥分区，战术条与增援坞对齐指挥台背板；补给/士气/经验/威胁与属性条统一为指挥台卡片样式；目标计划、安全接敌、战术命令与将领面板同步提升可读性；敌方意图与反制建议面板统一为指挥台卡片；反制下一步提示与关联 AI 行动行强化可执行反馈；态势响应卡与 AI 回放控制/当前事件反馈强化，让顶栏、地图容器、HUD 背板、状态芯片、侧栏容器、地形格、据点标签、单位棋子、攻击目标、战斗预测、普通攻击结果、态势分区和 AI 回放分区共享战区指挥台视觉基线；这些样式只影响表现层，不参与规则判定。
