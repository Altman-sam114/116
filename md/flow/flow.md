...
- 维护 `BattlefieldTheme`、通用 tactical surface 表现样式、地图材质 helper、战斗反馈卡、战线态势指挥简报和 AI 战况回放样式、地图标记槽位折叠，以及命令预览共享 `MapCommandPreviewChrome` helper；战术/占领/部署/整补结果卡与战斗结果卡共享战报卡视觉基线；反制执行回放与敌方回合复核卡同样对齐该基线，让顶栏、地图容器、HUD 背板、状态芯片、侧栏容器、地形格、据点标签、单位棋子、攻击目标、战斗预测、普通攻击结果、态势分区和 AI 回放分区共享战区指挥台视觉基线；这些样式只影响表现层，不参与规则判定。
