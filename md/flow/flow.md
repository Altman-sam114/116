...
- 显示 MOVE、ATK、POS、NEXT、OBJ、CAP、THR、INT、AI 复盘、战线态势汇总、定位入口、下一步提示、执行反馈、敌方回合影响和关联 AI 复盘入口、补给线、控制区、攻击覆盖等标记。
...
9. `BattlefieldSituationResponseSummary` 是最近一次态势响应的只读展示模型；`GameState.battlefieldSituationResponseSummary` 优先从 `latestEnemyThreatCountermeasureFollowUpResult` 派生敌方回合影响，其次从 `latestEnemyThreatCountermeasureExecutionResult` 派生反制执行反馈，再其次从 `latestObjectiveCaptureResult` 派生占点反馈，不新增 `@Published` 状态。
...
11. 反制执行响应显示建议类型、执行单位、入口类型、首条预计/实际对照和执行坐标；占点响应显示占领/夺取据点、归属变化、奖励和据点进度。普通攻击、普通移动、部署、整补或战术命令若没有对应最新复核、反制回放或占点结果，不会伪造态势响应。
