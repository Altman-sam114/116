...
用户输入 / 快捷按钮
  ...
  -> GameState handleTap / handlePrimaryAction / handleSecondaryAction / executeFocusedCommand / focusEnemyThreatCountermeasure / focusBattlefieldSituationPrimaryTarget / focusAIPhaseTimelineEvent / focusPreviousAIPhaseTimelineEvent / focusNextAIPhaseTimelineEvent / toggleAIPhaseTimelinePlayback / advanceAIPhaseTimelinePlayback
  -> GameModels 中的 Scenario / BattleUnit / TerrainTile / HexCoordinate / MapCommandPreview / BattlefieldSituationSummary / BattlefieldSituationResponseSummary
  -> GameState 规则判定与状态更新
  -> @Published 状态变化
...
- 显示 MOVE、ATK、POS、NEXT、OBJ、CAP、THR、INT、AI 复盘、战线态势汇总、定位入口、下一步提示、执行反馈和敌方回合影响、补给线、控制区、攻击覆盖等标记。
...
1. `BattlefieldSituationSummary` 是当前行动阵营的只读玩家回合态势模型，汇总指令点、待命部队、据点进度、敌方意图数量、攻击威胁数量、据点威胁数量、可执行反制数量、受威胁据点、首要建议、首要定位目标和下一步提示。
...
13. `ContentView` 在侧栏靠前显示“战线态势”卡，展示 summary 的等级、指标、受威胁据点、建议说明、态势响应反馈/敌方回合影响、“定位”按钮和下一步入口提示；UI 不重新计算威胁、反制、据点、OBJ 计划、待命单位、入口类型、执行反馈或复核影响。
