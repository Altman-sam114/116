...
- 显示 MOVE、ATK、POS、NEXT、OBJ、CAP、THR、INT、AI 复盘、战线态势汇总、定位入口、下一步提示和执行反馈、补给线、控制区、攻击覆盖等标记。
...
9. `BattlefieldSituationResponseSummary` 是最近一次态势响应的只读展示模型；`GameState.battlefieldSituationResponseSummary` 优先从 `latestEnemyThreatCountermeasureExecutionResult` 派生反制执行反馈，其次从 `latestObjectiveCaptureResult` 派生占点反馈，不新增 `@Published` 状态。
10. 反制响应显示建议类型、执行单位、入口类型、首条预计/实际对照和执行坐标；占点响应显示占领/夺取据点、归属变化、奖励和据点进度。普通攻击、普通移动、部署、整补或战术命令若没有对应最新反制回放或占点结果，不会伪造态势响应。
11. 读取 `battlefieldSituationResponseSummary` 不调用移动、攻击、战术命令、部署、整补、AI、日志写入或任何真实执行方法，不改变单位、据点、指令点、战报、消息或最新结果。
12. `ContentView` 在侧栏靠前显示“战线态势”卡，展示 summary 的等级、指标、受威胁据点、建议说明、态势响应反馈、“定位”按钮和下一步入口提示；UI 不重新计算威胁、反制、据点、OBJ 计划、待命单位、入口类型或执行反馈。
